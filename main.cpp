...
void push_message(byte type, uint32_t lval, float fval, const char* sval) {
  ...
  int ifttt_enable = os.options[OPTION_IFTTT_ENABLE]&test_type;
  ...
  if (ifttt_enable) {
    ...
    if(strlen(key)==0) ifttt_enable = 0;
  ...
  if (os.mqtt_enable && strlen(topic) & strlen(payload)) os.mqtt_publish(topic, payload);

  if (!ifttt_enable) return;
